...
class TestInviteReplaceOnTcp < DrivenSipTestCase

  def setup
    SipperConfigurator[:ControllerPath] = nil
    @tp = SipperConfigurator[:LocalSipperTransports]
    SipperConfigurator[:LocalSipperTransports] = ["tcp"]
    super
    str = <<-EOF
    
    require 'sip_test_driver_controller'
    
    module TestInviteReplaceOnTcp_SipInline
      class BobLabAndDeskController < SIP::SipTestDriverController
        ...
        def on_invite(bob_session1)
          ...
          bob_session2 = create_tcp_session(SipperConfigurator[:LocalSipperIP], SipperConfigurator[:LocalTestPort])
      ...
      class AliceController < SIP::SipTestDriverController
        ...
        def start
          alice_session1 = create_tcp_session(SipperConfigurator[:LocalSipperIP], SipperConfigurator[:LocalTestPort])
        ...
        def on_ack(alice_session2)
          ...
          alice_session2.flow_completed_for("TestInviteReplaceOnTcp")
        end
    ...
    set_controller("TestInviteReplaceOnTcp_SipInline::AliceController")
  end
  
  
  def test_invite_replace_on_tcp
    # alice_session1
