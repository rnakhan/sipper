#ifndef __SIPPER_MEDIA_LOCK_H__
#define __SIPPER_MEDIA_LOCK_H__
...
class SipperMediaMutex
{
   ...
   public:
      ...
      SipperMediaMutex(MutexType type) 
      {
         _initType(type);
      }
...
class SipperMediaLock
{
   private:

      const SipperMediaMutex *_mutex;

   public:

      SipperMediaLock(const SipperMediaMutex *mutex) :
         _mutex(mutex)
      {
         if(_mutex)
         {
...
#define MutexGuard(X) SipperMediaLock __locLock(X)
